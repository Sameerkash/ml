...
class Neural_Network(object):
    ...
    def backward(self, X, y, o, lr):
        ...
        self.W1 += X.T.dot(self.z2_delta) * lr      # adjusting first set (input --> hidden) weights
        self.W2 += self.z2.T.dot(self.o_delta) * lr # adjusting second set (hidden --> output) weights
...
NN = Neural_Network()
for i in tqdm(range(1000), unit='epochs',):
    # print ("\nInput: \n" + str(X))
    # print ("\nActual Output: \n" + str(y)) 
    # print ("\nPredicted Output: \n" + str(NN.forward(X)))
